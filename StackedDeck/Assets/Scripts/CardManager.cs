    ...
    void GenerateRandomDeck()
    {
        ...
        while(numR1 + numR2 < 40)
        {
            ...
            if(index == 0 && numR1 < totalCardsFromR1)
            {
                do
                {
                    c = regions[index].GetRandomCardAndAmount((numChampions >= 6), 6 - numChampions, totalCardsFromR1 - numR1, out isChampion);
                    ...
                    { 
                        if(cardCodeAndAmount[c.CardCode] < 3)
                        {
                            ...
                            c.Count = (c.Count < 3 - remainingNum ? c.Count : remainingNum);
            ...
            else if(numR2 < totalCardsFromR2)
            {
                do
                {
                    c = regions[index].GetRandomCardAndAmount((numChampions >= 6), 6 - numChampions, totalCardsFromR2 - numR2, out isChampion);
                    if (cardCodeAndAmount.ContainsKey(c.CardCode))
                    {
                        if (cardCodeAndAmount[c.CardCode] < 3)
                        {
